...
func (c *jobsController) GetAllJobs(ctx *gin.Context) {
	...
	if user.Role != "admin" {
		response := helpers.BuildErrorResponse("failed to process request", "failed to get all jobs, role is not admin", helpers.EmptyObj{})
		ctx.JSON(http.StatusBadRequest, response)
		return
	}
...
func (c *jobsController) GetJobsByID(ctx *gin.Context) {
	...
	token, errToken := c.jwtService.ValidateToken(authHeader)
	if errToken != nil {
		response := helpers.BuildErrorResponse("failed to process request", "failed to access jobs by id, token user admin wrong or empty", helpers.EmptyObj{})
		ctx.JSON(http.StatusBadRequest, response)
		return
	}
	...
	userID, err := strconv.Atoi(fmt.Sprintf("%v", claims["user_id"]))
	if err != nil {
		messError := fmt.Sprintf("failed to get jobs by id, user admin with user id %v is empty", userID)
		response := helpers.BuildErrorResponse("failed to process request", messError, helpers.EmptyObj{})
		ctx.JSON(http.StatusBadRequest, response)
		return
	}
...
func (c *jobsController) CreatedJobs(ctx *gin.Context) {
	...
	token, errToken := c.jwtService.ValidateToken(authHeader)
	if errToken != nil {
		response := helpers.BuildErrorResponse("failed to process request", "failed to create new job, token user admin wrong or empty", helpers.EmptyObj{})
		ctx.JSON(http.StatusBadRequest, response)
		return
	}
	claims := token.Claims.(jwt.MapClaims)
